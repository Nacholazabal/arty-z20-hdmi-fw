...
/* Maximum poll iterations before declaring a VDMA busy-wait timeout */
#define DISPLAY_VDMA_TIMEOUT_COUNT 0x00500000U
...
int DisplayStop(DisplayCtrl *dispPtr)
{
	 ...
	 */
	{
		...
		{
			if (++vdmaTimeout >= DISPLAY_VDMA_TIMEOUT_COUNT)
			{
				xil_printf("[ERR ] DisplayStop: VDMA read channel stop timeout - forcing state STOPPED\r\n");
				break;
			}
		}
	}
...
int DisplayStart(DisplayCtrl *dispPtr)
{
    ...
    */
	if (ClkStop(dispPtr->dynClkAddr))
	{
		xil_printf("[ERR ] DisplayStart: ClkStop timed out before reconfiguring clock\r\n");
		return XST_FAILURE;
	}
	if (ClkStart(dispPtr->dynClkAddr))
	{
		xil_printf("[ERR ] DisplayStart: ClkStart timed out - PLL cannot lock for mode: %s\r\n", dispPtr->vMode.label);
		return XST_FAILURE;
	}
...
int DisplayInitialize(DisplayCtrl *dispPtr, XAxiVdma *vdma, u16 vtcId, u32 dynClkAddr, u8 *framePtr[DISPLAY_NUM_FRAMES], u32 stride)
{
    ...
    */
	if (ClkStart(dispPtr->dynClkAddr))
	{
		xil_printf("[ERR ] DisplayInitialize: ClkStart timed out - check dynclk base address 0x%08X\r\n", dynClkAddr);
		return XST_FAILURE;
	}
